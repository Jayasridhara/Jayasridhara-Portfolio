...
const Typewriter = React.memo(({ text, delay = 0 }) => {
  ...
  return (
    <span className="font-mono relative">
      ...
      {/* Note: CSS for blink and fadeInUp is now handled in the main App component's style block */}
    </span>
  );
});
    ...
    <Section id="home" className="bg-gray-900 relative pt-32 overflow-hidden">
      ...
      <div className="relative z-10 grid grid-cols-1 gap-12 items-center">
        ...
        <div className="max-w-3xl">
          ...
          {/* This element uses the global fadeInUp style with a manually applied delay */}
 ...
 const ProjectCard = ({ title, description, techStack, githubLink, demoLink, image }) => (
  <div className="bg-gray-800 rounded-xl shadow-2xl overflow-hidden border border-gray-700 transition duration-500 hover:shadow-teal-500/20 transform hover:-translate-y-1 flex flex-col">
    ...
    <div className="p-6 flex flex-col grow">
      ...
      <p className="text-gray-400 text-md mb-4 grow">{description}</p>
  ...
  },
  {
    ...
    image: "https://placehold.co/600x400/14B8A6/FFFFFF?text=E-Commerce+Store"
  },
];
...
const Projects = () => (
  <Section id="projects" className="bg-gray-900 relative">
    <FadeInOnScroll animation="fadeInUp" className="text-center mb-16">
      ...
      <p className="text-lg text-gray-400">Some of the projects I'm most proud of.</p>
    </FadeInOnScroll>

    <div className="space-y-16">
      {projectsData.map((project, index) => (
        <FadeInOnScroll key={index} animation="fadeInUp" delay={150 * index}>
          <div className="relative group rounded-2xl overflow-hidden shadow-2xl border border-gray-700">
            {/* Background Image */}
            <img
              src={project.image}
              alt={`${project.title} Screenshot`}
              className="w-full h-[450px] object-cover transform group-hover:scale-105 transition-transform duration-700 ease-in-out"
            />

            {/* Overlay */}
            <div className="absolute inset-0 bg-linear-to-t from-gray-900/95 via-gray-900/60 to-transparent opacity-0 group-hover:opacity-100 transition duration-500 flex flex-col justify-end p-8">
              <h4 className="text-3xl font-bold text-white mb-3">{project.title}</h4>
              <p className="text-gray-300 mb-4 text-lg max-w-3xl">{project.description}</p>
      ...
      <Section id="contact" className="bg-gray-800">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-4xl mx-auto">
          ...
          <FadeInOnScroll animation="fadeInLeft" delay={200} className="flex flex-col space-y-6 text-gray-300">
            ...
            {/* Added abstract image placeholder */}
            <div className="pt-4">
                <img 
                    src="https://placehold.co/400x150/10B981/1F2937?text=Abstract+Tech+Visual" 
                    alt="Abstract technology graphic" 
                    className="rounded-lg shadow-lg border border-teal-600/50"
                />
            </div>
          </FadeInOnScroll>
